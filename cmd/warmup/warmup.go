...
func init() {
	...
	enuygunCmd.Flags().IntVarP(&maxWorker, "max-worker", "w", 10, "Maximum number of workers")
	enuygunCmd.Flags().IntVarP(&maxRequestInTime, "max-request-in-time", "r", 600, "Maximum number of requests in time")
	enuygunCmd.Flags().IntVarP(&maxRunTime, "max-run-time", "s", 60, "The time limit for the specified limit in seconds")

	rootCmd.AddCommand(enuygunCmd)
}
